...
**As fases 2 a 4 continuam não implementadas.** Não existe ainda nenhuma porta de entrada HTTP para montar uma árvore composta: sem porta JSON (`QuerySpec<T>.Filter`, fase 2 — Etapa 4 do plano), sem mini-linguagem para query string (fase 3 — Etapa 5), sem limites de profundidade/tamanho de árvore nem validação com 400 (fase 4 — Etapa 6). O comportamento observável de `Filters` (`Dictionary<string,string>`, combinação exclusiva por `AND` do ponto de vista de quem só usa esse caminho) segue idêntico ao de antes da Etapa 2 — a limitação de não haver `OR`/agrupamento/`NOT` continua real **para quem usa `Filters`**, mas passou a ser uma limitação só desse formato de transporte, não mais do motor de compilação por baixo (que já sabe compilar árvores compostas, só não tem porta de entrada pública para recebê-las ainda).
