...
**As fases 3 e 4 continuam não implementadas.** Não há ainda mini-linguagem para expressar `OR`/agrupamento/`NOT` na própria query string (fase 3 — Etapa 5), nem limites de profundidade/tamanho de árvore ou validação com 400 para a árvore JSON (fase 4 — Etapa 6). O comportamento observável de `Filters`/`QueryFilter` sozinhos (sem `Filter`) segue idêntico ao de antes da Etapa 2 — combinação exclusiva por `AND` — mas isso já não é mais uma limitação do motor de compilação, e sim só desses dois formatos de transporte especificamente; quem precisa de `OR`/agrupamento/`NOT` hoje usa a porta JSON via `Filter`.
