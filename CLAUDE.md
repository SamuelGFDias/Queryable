...
**A fase 4 (limites de profundidade/tamanho + validação com 400) também está implementada** (Etapa 6 do plano). Com isso, **as quatro fases da proposta de filtros compostos estão concluídas.** `FilterLimits`, `FilterLimitValidator` e `FilterLimitExceededException` (`src/Queryable/Filtering/`) formam os tetos de segurança aplicados a uma árvore `FilterNode` vinda de cliente externo, antes de qualquer compilação para `Expression`. `FilterLimits` tem quatro propriedades mutáveis, todas com default (seção 7 da proposta): `MaxDepth = 6` (profundidade de aninhamento de `FilterGroup`/`FilterNot`), `MaxNodes = 100` (total de nós na árvore, somados recursivamente), `MaxExpressionLength = 4096` (tamanho em caracteres da string da mini-linguagem) e `MaxInItems = 200` (itens na lista CSV de uma condição `in`). `FilterLimitExceededException` é uma especialização de `ArgumentException` com `Limit` (`FilterLimitKind`), `Found` e `Allowed`.

`FilterLimitValidator.Validate` é **iterativo de propósito, com uma `Stack<T>` explícita no heap, não recursivo.** Um validador recursivo estouraria a própria pilha de chamadas exatamente na árvore patológica que ele existe para rejeitar — trocaria um `400` previsível (`FilterLimitExceededException`) por um `StackOverflowException` não recuperável no processo. Isso é o mesmo raciocínio, aplicado a uma nova classe, por trás da guarda de ciclo por caminho de `PathExtension` (ver "Arquitetura" acima), mas aqui o vetor de ataque é profundidade de árvore controlada pelo cliente, não navegação de propriedades.

Quatro pontos de validação, cada um cobrindo uma origem: (1) `FilterExpressionParser.Parse` verifica `MaxExpressionLength` sobre a string bruta, **antes de tokenizar** — rejeita por tamanho mesmo uma entrada sintaticamente inválida, sem gastar tempo de parsing nela; ao final, valida a árvore montada com `FilterLimitValidator.Validate` antes de devolvê-la. (2) `FilterNodeJsonConverter.Read` valida a árvore completa logo após desserializá-la, antes de devolvê-la ao chamador — é o único ponto que cobre **todo** `FilterNode` que passa pelo `System.Text.Json`, incluindo um `[FromBody] QuerySpec<T>` desserializado direto pelo model binder de JSON do ASP.NET Core, sem passar por `QuerySpecModelBinder<T>.BuildSpec` nem por `RequestQueryExtensions.ToQuerySpec`. (3) `QuerySpecModelBinder<T>.BuildSpec` revalida `spec.Filter` já combinado (proveniente do parse de `filter`) ao final. (4) `RequestQueryExtensions.ToQuerySpec<T>()` revalida `spec.Filter` já combinado — `CombineFilters` soma `RequestQuery.Filter` (árvore JSON) com o resultado do parse de `RequestQuery.FilterExpression` em `FilterGroup(And, [...])` antes da revalidação final. Os pontos (3) e (4) existem porque `Filter` e `FilterExpression` podem, cada um isoladamente, caber nos limites e a combinação estourar um teto — só a árvore final já combinada garante cobertura.

`FilterNodeJsonConverter` valida sempre contra `FilterLimits.Default`, nunca contra limites configurados via DI — `JsonConverter<T>` é instanciado pelo `System.Text.Json` sem receber `IServiceProvider`, então não há como injetar o `FilterLimits` registrado por `AddQueryableDynamicFilter(Action<FilterLimits>)` nesse ponto. Isso é compensado pelos pontos (3)/(4) acima, que resolvem `FilterLimits` do DI e revalidam a árvore final — mas só protegem contra limites **mais restritivos** que os defaults; um `MaxNodes` configurado **maior** que 100 não adianta para um corpo JSON, porque o conversor já barrou pelos defaults antes de a árvore chegar ao binder ou a `ToQuerySpec`.

`QuerySpecModelBinder<T>.BindModelAsync` resolve `FilterLimits` de `HttpContext.RequestServices` (caindo para `FilterLimits.Default` se nada estiver registrado) e captura `FilterExpressionSyntaxException`/`FilterLimitExceededException`, convertendo-as em `ModelState`/`ModelBindingResult.Failed()` — com `[ApiController]`, isso vira `400` automaticamente. `BuildSpec`, em contraste, continua **lançando** as duas exceções — decisão deliberada para que a lógica de montagem do `QuerySpec<T>` permaneça testável sem infraestrutura web (`HttpContext`/`ModelBindingContext` fake); é `BindModelAsync`, a borda HTTP, quem decide converter em erro de modelo, não `BuildSpec`.
